...
class QaInspection(models.Model):
    _name = 'qa.inspection'
    _description = 'Quality assurance inspection'
    ...
    test = fields.Many2one(
        comodel_name='qa.test', string='Test', readonly=True, select=True)
    inspection_lines = fields.One2many(
        comodel_name='qa.inspection.line', inverse_name='inspection_id',
    ...
    def create(self, vals):
        if vals.get('name', '/') == '/':
            vals['name'] = self.env['ir.sequence'].get('qa.inspection')
        return super(QaInspection, self).create(vals)
            ...
            if inspection.state != 'draft':
                raise exceptions.Warning(
                    _("You cannot remove an inspection that is not in draft "
                      "state."))
        return super(QaInspection, self).unlink()
...
class QaInspectionLine(models.Model):
    _name = 'qa.inspection.line'
    _description = "Quality assurance inspection line"
    ...
    inspection_id = fields.Many2one(
        comodel_name='qa.inspection', string='Inspection', ondelete='cascade')
    ...
    test_line = fields.Many2one(
        comodel_name='qa.test.question', string='Test question',
        readonly=True)
    possible_ql_values = fields.Many2many(
        comodel_name='qa.test.question.value', string='Answers')
    ...
    qualitative_value = fields.Many2one(
        comodel_name='qa.test.question.value', string='Qualitative value',
