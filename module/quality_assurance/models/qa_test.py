...
class QaTest(models.Model):
    ...
    """
    _name = 'qa.test'
    _description = 'Quality assurance test'
    ...
    test_lines = fields.One2many(
        comodel_name='qa.test.question', inverse_name='test',
    ...
    category = fields.Many2one(
        comodel_name='qa.test.category', string='Category')
...
class QaTestQuestion(models.Model):
    """Each test line is a question with its valid value(s)."""
    _name = 'qa.test.question'
    _description = 'Quality assurance question'
    _order = 'sequence, id'
    ...
    test = fields.Many2one(comodel_name='qa.test', string='Test')
    name = fields.Char(
    ...
    ql_values = fields.One2many(
        comodel_name='qa.test.question.value', inverse_name="test_line",
...
class QaTestQuestionValue(models.Model):
    _name = 'qa.test.question.value'
