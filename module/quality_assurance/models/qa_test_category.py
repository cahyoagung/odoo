...
class QaTestTemplateCategory(models.Model):
    _name = 'qa.test.category'
    ...
    parent_id = fields.Many2one(
        comodel_name='qa.test.category', string='Parent category', select=True)
    complete_name = fields.Char(
        compute="_get_complete_name", string='Full name')
    child_ids = fields.One2many(
        comodel_name='qa.test.category', inverse_name='parent_id',
